...
- **`pkg-release.yml`** gains a `changelog-enrich` sibling job (`mode: finalise`, `secrets: inherit`) calling `reusable-changelog-enrich.yml` ([A-821](https://linear.app/rheged-studio/issue/A-821) pin) so post-merge fields and release `version` stamps land in-repo as `road-runner-bot[bot]`.

## Fixed

N/A — canary adoption of the shared enricher path (parity vs the orchestrator inline finalise is the [A-796](https://linear.app/rheged-studio/issue/A-796) gate).
